...
def deal_left_car(cars_list):
    ...
    for car in left_cars:
        ...
        # car_buffer
        total_task -= len(car_buffer[car])
        del car_buffer[car]
        # bus_buffer
        total_task -= sum(1 for task_packet_list in bus_buffer.values() for task_packet in task_packet_list if task_packet[0][0] == car)
        for bus in bus_buffer:
            bus_buffer[bus] = [task_packet for task_packet in bus_buffer[bus] if task_packet[0][0] != car]
        # MEC_buffer
        total_task -= sum(1 for task_packet_list in MEC_buffer.values() for task_packet in task_packet_list if task_packet[0][0] == car)
        for MEC in MEC_buffer:
            MEC_buffer[MEC] = [task_packet for task_packet in MEC_buffer[MEC] if task_packet[0][0] != car]
        # bus_return_list
...
def offload_decision():
    ...
    for car_id, task_list in task_queue.items():
        ...
        task_list.sort(key=lambda x: (x[1], x[2], x[4] + x[5] - current_time, x[4]))
        for task in task_list:
            if current_time >= task[4] + task[5]:  # 若任務已超時則丟棄
                ...
                if task[1] == 0:
                    local_queue_delay = sum(math.ceil((task_packet[1][2] - current_time) * 100) / 100 for task_packet in car_process_list[car_id] if task_packet[0][1] == 0) + sum(math.ceil(task_packet[1][1] * 100) / 100 for task_packet in car_receive_queue[car_id] if task_packet[0][1] == 0)
                elif task[1] == 1:
                    local_queue_delay = sum(math.ceil((task_packet[1][2] - current_time) * 100) / 100 for task_packet in car_process_list[car_id] if task_packet[0][1] in (0,1)) + sum(math.ceil(task_packet[1][1] * 100) / 100 for task_packet in car_buffer[car_id] if task_packet[0][1] == 1) + \
                        sum(math.ceil(task_packet[1][1] * 100) / 100 for task_packet in car_receive_queue[car_id] if task_packet[0][1] in (0,1))
                else:
                    local_queue_delay = sum(math.ceil((task_packet[1][2] - current_time) * 100) / 100 for task_packet in car_process_list[car_id]) + sum(math.ceil(task_packet[1][1] * 100) / 100 for task_packet in car_buffer[car_id]) + \
                        sum(math.ceil(task_packet[1][1] * 100) / 100 for task_packet in car_receive_queue[car_id])
               # 滿足條件則本地計算(總延遲 < 剩餘時間 and 不用交給MEC計算)
                if (math.ceil(task[2] / car_computation_capacity * 100) / 100 + local_queue_delay < task[4] + task[5] - current_time) and task[6] == 0:
                    ...
                    for bus in candidate_buses.keys():
                        transmit_latency_score = 1 - (candidate_buses[bus][0] / task[5])
                        compute_latency_score = 1 - (candidate_buses[bus][1] / task[5])
                        queue_latency_score = 1 - (candidate_buses[bus][2] / task[5])
                        total_score = x * transmit_latency_score + y * compute_latency_score + z * queue_latency_score
                        candidate_list.append([bus, total_score, candidate_buses[bus][0], candidate_buses[bus][1]])
                    for MEC in candidate_MECs.keys():
                        transmit_latency_score = 1 - (candidate_MECs[MEC][0] / task[5])
                        compute_latency_score = 1 - (candidate_MECs[MEC][1] / task[5])
                        queue_latency_score = 1 - (candidate_MECs[MEC][2] / task[5])
                        total_score = x * transmit_latency_score + y * compute_latency_score + z * queue_latency_score
                        candidate_list.append([MEC, total_score, candidate_MECs[MEC][0], candidate_MECs[MEC][1]])
                    # 選取分數最高的節點來卸載
                    candidate_list.sort(key=lambda x: x[1], reverse=True)
                    if candidate_list and candidate_list[0][1] > 0.7:
                        timestamp = current_time
                        transmit_latency = candidate_list[0][2]
                        compute_latency = candidate_list[0][3]
                        finish_time = 0
                        if isinstance(candidate_list[0][0], str): # 公車
                            car_V2V_transmit_list[car_id].append([task, [timestamp, candidate_list[0][0], transmit_latency, compute_latency, finish_time]])
                        else: # MEC
                            car_V2I_transmit_list[car_id].append([task, [timestamp, candidate_list[0][0], transmit_latency, compute_latency, finish_time]])
                    else:
                        temp_list.append(task)
...
def get_candidate_buses(task, nearby_buses_list):
    ...
    for bus in nearby_buses_list: 
        ...
        if task[1] == 0:
            bus_queue_delay = sum(math.ceil((task_packet[1][4] - current_time) * 100) / 100 for task_packet in bus_process_list[bus] if task_packet[0][1] == 0) + sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in bus_receive_queue[bus] if task_packet[0][1] == 0)
        elif task[1] == 1:
            bus_queue_delay = sum(math.ceil((task_packet[1][4] - current_time) * 100) / 100 for task_packet in bus_process_list[bus] if task_packet[0][1] in (0,1)) + sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in bus_buffer[bus] if task_packet[0][1] == 1) + \
                sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in bus_receive_queue[bus] if task_packet[0][1] in (0,1))
        else:
            bus_queue_delay = sum(math.ceil((task_packet[1][4] - current_time) * 100) / 100 for task_packet in bus_process_list[bus]) + sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in bus_buffer[bus]) + \
                sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in bus_receive_queue[bus])
        ...
        if total_delay < task[4] + task[5] - current_time and predict_stayTime > total_delay:
            candidate_buses[bus] = [transmission_delay, computation_delay, bus_queue_delay]
...
def get_candidate_MECs(task, car_nearby_MECs):
    ...
    for MEC in car_nearby_MECs: 
        ...
        if task[1] == 0:
            MEC_queue_delay = sum(math.ceil((task_packet[1][4] - current_time) * 100) / 100 for task_packet in MEC_process_list[MEC] if task_packet[0][1] == 0) + sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in MEC_receive_queue[MEC] if task_packet[0][1] == 0)
        elif task[1] == 1:
            MEC_queue_delay = sum(math.ceil((task_packet[1][4] - current_time) * 100) / 100 for task_packet in MEC_process_list[MEC] if task_packet[0][1] in (0,1)) + sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in MEC_buffer[MEC] if task_packet[0][1] == 1) + \
                sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in MEC_receive_queue[MEC] if task_packet[0][1] in (0,1))
        else:
            MEC_queue_delay = sum(math.ceil((task_packet[1][4] - current_time) * 100) / 100 for task_packet in MEC_process_list[MEC]) + sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in MEC_buffer[MEC]) + \
                sum(math.ceil(task_packet[1][3] * 100) / 100 for task_packet in MEC_receive_queue[MEC])
        total_delay = MEC_queue_delay + 2 * math.ceil(transmission_delay * 100) / 100 + math.ceil(computation_delay * 100) / 100
        # 如果能在過期前完成且停留時間大於總延遲
        if total_delay < task[4] + task[5] - current_time and predict_stayTime > total_delay:
            candidate_MECs[MEC] = [transmission_delay, computation_delay, MEC_queue_delay]
...
def choose_task_to_process(cars_list, buses_list):
    ...
    for car in cars_list:
        ...
        # 根據優先級、所需運算時間、抵達queue時間做排序
        car_receive_queue[car].sort(key=lambda x: (x[0][1], x[0][2], x[0][4] + x[0][5] - current_time, x[1][0]))

        # 將buffer中過期的任務移除
        buffer_temp = [] # 儲存沒過期的任務
        for task_packet in car_buffer[car]:
            # 過期
            if current_time >= task_packet[0][4] + task_packet[0][5]:
                fail_recorder[3][task_packet[0][1]] += 1
            # 沒過期
            else:
                buffer_temp.append(task_packet)
        car_buffer[car] = buffer_temp

        # 若沒有任務在執行
        if not car_process_list[car]:
            # 若沒有被中斷的任務
            if not car_buffer[car]:
                # 若queue中有任務在排隊，則執行第一個任務
                if car_receive_queue[car]:
            ...
            # 若有被中斷的任務
                ...
                # 若queue中沒有任務在排隊，則執行buffer的第一個任務
                if not car_receive_queue[car]:
                    task_packet = car_buffer[car][0]
                    # 將封包完成時間設為目前時間點 + 計算延遲
                    task_packet[1][2] = current_time + task_packet[1][1]
                    car_process_list[car].append(task_packet)
                    car_buffer[car].pop(0)
                # 若queue中有任務在排隊
                else:
                    # 若queue中的第一個任務的優先級大於buffer中第一個任務的優先級，則執行queue中第一個任務
                    if car_receive_queue[car][0][0][1] < car_buffer[car][0][0][1]:
                        task_packet = car_receive_queue[car][0]
                        # 將封包完成時間設為目前時間點 + 計算延遲
                        task_packet[1][2] = current_time + task_packet[1][1]
                        car_process_list[car].append(task_packet)
                        car_receive_queue[car].pop(0)
                    # 若queue中的第一個任務的優先級小於等於buffer中第一個任務的優先級，則執行buffer中第一個任務
                    else:
                        task_packet = car_buffer[car][0]
                        # 將封包完成時間設為目前時間點 + 計算延遲
                        task_packet[1][2] = current_time + task_packet[1][1]
        ...
        # 若有任務正在執行
            ...
            # 不管buffer有沒有被中斷的任務，若queue中有任務在排隊
            if car_receive_queue[car]:
                # 若queue中的第一個任務的優先級大於正在執行的任務的優先級，則將被中斷的任務放入buffer，並對buffer進行優先級排序，然後執行queue中第一個任務
                if car_receive_queue[car][0][0][1] < car_process_list[car][0][0][1]:
                    interrupted_task_packet = car_process_list[car][0]
                    # 將被中斷的任務之運算延遲扣除已經運算好的部分
                    interrupted_task_packet[1][1] = interrupted_task_packet[1][2] - current_time
                    car_buffer[car].append(interrupted_task_packet)
                    car_buffer[car].sort(key=lambda x: x[0][1])
                    car_process_list[car].pop(0)
    ...
    for bus in buses_list:
        ...
        # 根據優先級、所需運算時間、抵達queue時間做排序
        bus_receive_queue[bus].sort(key=lambda x: (x[0][1], x[0][2], x[0][4] + x[0][5] - current_time, x[1][0] + x[1][2]))

        # 將buffer中過期的任務移除
        buffer_temp = [] # 儲存沒過期的任務
        for task_packet in bus_buffer[bus]:
            # 過期
            if current_time >= task_packet[0][4] + task_packet[0][5]:
                fail_recorder[4][task_packet[0][1]] += 1
            # 沒過期
                ...
                buffer_temp.append(task_packet)
        bus_buffer[bus] = buffer_temp

        # 若沒有任務在執行
        if not bus_process_list[bus]:
            # 若沒有被中斷的任務
            if not bus_buffer[bus]:
                # 若queue中有任務在排隊，則執行第一個任務
                if bus_receive_queue[bus]:
            ...
            # 若有被中斷的任務
                ...
                # 若queue中沒有任務在排隊，則執行buffer的第一個任務
                if not bus_receive_queue[bus]:
                    task_packet = bus_buffer[bus][0]
                    # 將封包完成時間設為目前時間點 + 計算延遲
                    task_packet[1][4] = current_time + task_packet[1][3]
                    bus_process_list[bus].append(task_packet)
                    bus_buffer[bus].pop(0)
                # 若queue中有任務在排隊
                else:
                    # 若queue中的第一個任務的優先級大於buffer中第一個任務的優先級，則執行queue中第一個任務
                    if bus_receive_queue[bus][0][0][1] < bus_buffer[bus][0][0][1]:
                        task_packet = bus_receive_queue[bus][0]
                        # 將封包完成時間設為目前時間點 + 計算延遲
                        task_packet[1][4] = current_time + task_packet[1][3]
                        bus_process_list[bus].append(task_packet)
        ...
        # 若有任務正在執行
            ...
            if bus_receive_queue[bus]:
                # 若queue中的第一個任務的優先級大於正在執行的任務的優先級，則將被中斷的任務放入buffer，並對buffer進行優先級排序，然後執行queue中第一個任務
                if bus_receive_queue[bus][0][0][1] < bus_process_list[bus][0][0][1]:
                    interrupted_task_packet = bus_process_list[bus][0]
                    # 將被中斷的任務之運算延遲扣除已經運算好的部分
                    interrupted_task_packet[1][3] = interrupted_task_packet[1][4] - current_time
                    bus_buffer[bus].append(interrupted_task_packet)
                    bus_buffer[bus].sort(key=lambda x: x[0][1])
                    bus_process_list[bus].pop(0)
    ...
    for MEC in range(MEC_quantity):
        ...
        # 根據優先級、所需運算時間、抵達queue時間做排序
        MEC_receive_queue[MEC].sort(key=lambda x: (x[0][1], x[0][2], x[0][4] + x[0][5] - current_time, x[1][0] + x[1][2]))

        # 將buffer中過期的任務移除
        buffer_temp = [] # 儲存沒過期的任務
        for task_packet in MEC_buffer[MEC]:
            # 過期
            if current_time >= task_packet[0][4] + task_packet[0][5]:
                fail_recorder[5][task_packet[0][1]] += 1
            # 沒過期
                ...
                buffer_temp.append(task_packet)
        MEC_buffer[MEC] = buffer_temp

        # 若沒有任務在執行
        if not MEC_process_list[MEC]:
            # 若沒有被中斷的任務
            if not MEC_buffer[MEC]:
                # 若queue中有任務在排隊，則執行第一個任務
                if MEC_receive_queue[MEC]:
            ...
            # 若有被中斷的任務
                ...
                # 若queue中沒有任務在排隊，則執行buffer的第一個任務
                if not MEC_receive_queue[MEC]:
                    task_packet = MEC_buffer[MEC][0]
                    # 將封包完成時間設為目前時間點 + 計算延遲
                    task_packet[1][4] = current_time + task_packet[1][3]
                    MEC_process_list[MEC].append(task_packet)
                    MEC_buffer[MEC].pop(0)
                # 若queue中有任務在排隊
                else:
                    # 若queue中的第一個任務的優先級大於buffer中第一個任務的優先級，則執行queue中第一個任務
                    if MEC_receive_queue[MEC][0][0][1] < MEC_buffer[MEC][0][0][1]:
                        task_packet = MEC_receive_queue[MEC][0]
                        # 將封包完成時間設為目前時間點 + 計算延遲
                        task_packet[1][4] = current_time + task_packet[1][3]
                        MEC_process_list[MEC].append(task_packet)
        ...
        # 若有任務正在執行
            ...
            if MEC_receive_queue[MEC]:
                # 若queue中的第一個任務的優先級大於正在執行的任務的優先級，則將被中斷的任務放入buffer，並對buffer進行優先級排序，然後執行queue中第一個任務
                if MEC_receive_queue[MEC][0][0][1] < MEC_process_list[MEC][0][0][1]:
                    interrupted_task_packet = MEC_process_list[MEC][0]
                    # 將被中斷的任務之運算延遲扣除已經運算好的部分
                    interrupted_task_packet[1][3] = interrupted_task_packet[1][4] - current_time
                    MEC_buffer[MEC].append(interrupted_task_packet)
                    MEC_buffer[MEC].sort(key=lambda x: x[0][1])
                    MEC_process_list[MEC].pop(0)
